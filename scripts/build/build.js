...
fs.readdir(path.join(functionsPath), (error, files) => {
    ...
    if (error) {
        console.warn(`\n Error reading files from ${functionsPath} -- @ scripts/build/build.js line 18 \n`);
        console.error(error);
    } else {
        if (!files.length) {
            console.error(`\n No files found in ${functionsPath} -- @ scripts/build/build.js line 18 \n`);
        } else {
            ...
            functions.forEach((fileName) => {
                fs.readFile(`${functionsPath}/${fileName}`, 'utf8', (error, file) => {
                    if (error) {
                        console.log(`\n\n ERROR @ scripts/build/build.js -- line 31 \n`)
...
// compile and write output to './prod/dist'
// const buildProdDist = function (file, outputPath, fileName) {
//     const babelOptions = {
//         filename: fileName,
//         comments: false,
//         code: true,
...
// generate and write output to './prod/index.js'
const buildProdIndex = function (file, outputPath, fileName) {

}

// generate and write output to 'prod/index.d.ts'
const buildProdTypeDeclaration = function (file, outputPath) {
